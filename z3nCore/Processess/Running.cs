...
namespace z3nCore
{
    ...
    public static class RunningObsolete
    {
        private static readonly string _mmfName = "ZennoRunningProcesses";
        private static readonly int _mmfSize = 1024 * 1024; // 1MB
        ...
        public static void Clear()
        {
            SaveAll(new Dictionary<int, List<object>>());
        }

        public static int Count
        {
            get { return LoadAll().Count; }
        }
        ...
        private static Dictionary<int, List<object>> LoadAll()
        {
            try
            {
                using (var mmf = MemoryMappedFile.OpenExisting(_mmfName))
        ...
        private static void SaveAll(Dictionary<int, List<object>> data)
        {
            try
            {
                string json = Global.ZennoLab.Json.JsonConvert.SerializeObject(data);
                ...
                if (jsonBytes.Length > _mmfSize - 4)
                    throw new Exception("Data too large for MMF");
        ...
        private static void CreateMMF()
        {
            try
            {
                var mmf = MemoryMappedFile.CreateNew(_mmfName, _mmfSize);
                // Не закрываем - оставляем открытым на весь процесс
            }
            catch
            {
                // MMF уже создан другим процессом
            }
        }
    }
