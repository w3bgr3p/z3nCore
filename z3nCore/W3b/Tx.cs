...
namespace z3nCore
{
    public class Tx_
    {
        ...
        public string SendTx(string chainRpc, string transactionJson, string walletKey = null, int txType = 2, int speedup = 1, bool debug = false)
        {
            ...
            try
            {
                ...
                try
                {
                    txData = Newtonsoft.Json.JsonConvert.DeserializeObject(transactionJson);
                }
                catch (Exception ex)
                {
        ...
        public string SendTx_(string chainRpc, string contractAddress, string encodedData, object value, string walletKey, int txType = 2, int speedup = 1, bool debug = false)
        {
            var report = new StringBuilder();
            ...
            {
                if (debug) _project.warn(report.ToString());
                throw new Exception($"Failed to get chain ID: {ex.Message}, InnerException: {ex.InnerException?.Message}", ex);
            }
            report.AppendLine($"chainId: {chainId}");
            string fromAddress;
    ...
    public class Tx
    {
        ...
        public string ReplaceInZerion(Instance instance, string chainRpc,string walletKey = null, int txType = 2, int speedup = 1, bool debug = false)
        {
