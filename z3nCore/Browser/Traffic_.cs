...
namespace z3nCore
{
    ...
    public class _Traffic
    {
        ...
        public _Traffic(IZennoPosterProjectModel project, Instance instance, bool log = false)
        {
            _project = project;
            _instance = instance;
            _showLog = log;
        ...
        public Dictionary<string, string> Get(string url, bool reload = false, bool strict = true, int deadline = 10)
        {
            _project.Deadline();
            _instance.UseTrafficMonitoring = true;
                ...
                if (strict)
                ...
                else
                    {
                        if (t.Url.Contains(url))
                        {
                            ...
                            var ResponseHeaders = t.ResponseHeaders;
                            var ResponseCookies = t.ResponseCookies;
                            var ResponseBody = t.ResponseBody == null
                                ? ""
                                : Encoding.UTF8.GetString(t.ResponseBody, 0, t.ResponseBody.Length);
