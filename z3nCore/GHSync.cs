...
public class GHsync
{
    ...
    public void Main(string baseDir, string token, string username)
    {
        ...
        try
        {
            ...
            _log.Send($"Found {totalFolders} folders");

            foreach (string subDir in subDirs)
            {
                if (_project.Lists["notExisting"].Contains(subDir))
                {
                    _log.Send($"[!REPO] (repo not exist): {subDir}");
                    foldersSkipped++;
                    continue;
                }
                ...
                try
                {
                    ...
                    foldersWithChanges++;
                    RunGit("add .", subDir);
                    RunGit($"commit -m \"{DateTime.UtcNow.ToString("o")}\"", subDir);
                ...
                catch (Exception ex)
                {
                    ...
                    if (ex.Message.Contains("not found"))
                        _project.Lists["notExisting"].Add(subDir);
        ...
        _log.Send($"Summary: Total={totalFolders}, Changes={foldersWithChanges}, Skipped={foldersSkipped}, SizeSkipped={foldersSkippedBySize}, Committed={successfullyCommitted}, Errors={errorCount}");
    }
